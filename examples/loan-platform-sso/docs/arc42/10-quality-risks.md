...
**Description:** The self-signed certificate generated by `nginx/gen-cert.sh` is
for `localhost` only and is not trusted by browsers without a manual exception.
All credentials (Keycloak admin, DB passwords, client secrets, cookie secret) are
hardcoded dev values.

**Impact:** The project cannot be deployed to a production or staging environment
without significant additional configuration (CA-signed certificate, secrets vault,
network hardening).
...
**Impact:** The Redoc page at `https://localhost:8080/engine-rest` would appear
