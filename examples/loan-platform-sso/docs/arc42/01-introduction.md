# 01 Introduction and Goals — loan-platform-sso

## Business Context

The **loan-platform-sso** example demonstrates a complete, secured loan-origination
...
(Flowset), all behind a single TLS endpoint.
...
| 4 | **Reproducibility** | A clean checkout + `docker compose up --build` reproduces the full running stack. No manual configuration beyond `gen-cert.sh`. |
...
| **Operaton developer** | Understand how to integrate Keycloak SSO with the Run distribution; reuse the pattern for their own projects. |
